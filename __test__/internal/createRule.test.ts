...
describe('createRule internal function', () => {
  ...
  test('should call callback function with test result', () => {
    const callback = jest.fn(() => null)
    const input = 'a + b = c'
    const test = (input: string) => expectedTestResult(input)
    const rule = createRule(test, callback)
    rule(input, 0)
    expect(callback).toHaveBeenCalledTimes(1)
    expect(callback).toHaveBeenCalledWith(expectedTestResult(input), 0)
  })
  ...
  test('should return callback result', () => {
    ...
    expect(result).toEqual(expectedTestResult(input))
  })
