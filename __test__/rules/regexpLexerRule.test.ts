import { regexpLexerRule, regexpRule, regexpTest } from '../../src'
import type { MultiTokenRuleResult } from '../../src/types/rule-types'

describe('regexpLexerRule function', () => {

  test('should be a function', () => {
    const rule = regexpLexerRule(/[\s\S]*/, [])
    expect(typeof rule === 'function').toBe(true)
  })

  test('should return falsy if input doesn\'t match', () => {
    const expressionRule = regexpLexerRule(/\{.*\}/, [
      regexpRule(/\d+/, 'Digits'),
